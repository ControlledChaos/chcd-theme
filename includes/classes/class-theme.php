...
class Theme {
	...
	public function __construct() {
		...
		// Skiip link.
		add_action( 'chcd_body_open', [ $this, 'skip_link' ], 5 );
	...
	public function setup() {
		...
		add_theme_support( 'custom-background', apply_filters( 'chcd_background', [
			'default-image'          => '',
		...
		register_default_headers(
			apply_filters(
				...
				[
					'chcd-theme' => [
						'url'           => get_theme_file_uri( 'assets/images/chcd.jpg' ),
						'thumbnail_url' => get_theme_file_uri( 'assets/images/chcd-thumb.jpg' ),
						'description'   => __( 'Wilbur Image', 'chcd-theme' ),
					],
				]
			)
		);
		 ...
		 * to change 'chcd-theme' to the name of your theme in all the template files.
		 */
		load_theme_textdomain( 'chcd-theme' );
		...
		if ( is_customize_preview() ) {
			...
			// add_theme_support( 'starter-content', chcd_get_starter_content() );
		}
		...
		$editor_color_palette = [
			[
				'name'  => __( 'Accent Color', 'chcd-theme' ),
				'slug'  => 'accent',
				'color' => $accent,
			],
			[
				'name'  => __( 'Primary', 'chcd-theme' ),
				'slug'  => 'primary',
				'color' => $primary,
			],
			[
				'name'  => __( 'Secondary', 'chcd-theme' ),
				'slug'  => 'secondary',
				'color' => $secondary,
			],
			[
				'name'  => __( 'Subtle Background', 'chcd-theme' ),
		...
		$editor_color_palette[] = [
			'name'  => __( 'Background Color', 'chcd-theme' ),
			'slug'  => 'background',
		...
		add_theme_support(
			...
			[
				[
					'name'      => _x( 'Small', 'Name of the small font size in the block editor', 'chcd-theme' ),
					'shortName' => _x( 'S', 'Short name of the small font size in the block editor.', 'chcd-theme' ),
					'size'      => 18,
					'slug'      => 'small',
				],
				[
					'name'      => _x( 'Regular', 'Name of the regular font size in the block editor', 'chcd-theme' ),
					'shortName' => _x( 'M', 'Short name of the regular font size in the block editor.', 'chcd-theme' ),
					'size'      => 21,
					'slug'      => 'normal',
				],
				[
					'name'      => _x( 'Large', 'Name of the large font size in the block editor', 'chcd-theme' ),
					'shortName' => _x( 'L', 'Short name of the large font size in the block editor.', 'chcd-theme' ),
					'size'      => 26.25,
					'slug'      => 'large',
				],
				[
					'name'      => _x( 'Larger', 'Name of the larger font size in the block editor', 'chcd-theme' ),
					'shortName' => _x( 'XL', 'Short name of the larger font size in the block editor.', 'chcd-theme' ),
					'size'      => 32,
					'slug'      => 'larger',
				],
			]
	...
	public function menus() {
		...
		$locations = [
			'primary'  => __( 'Desktop Horizontal Menu', 'chcd-theme' ),
			'expanded' => __( 'Desktop Expanded Menu', 'chcd-theme' ),
			'mobile'   => __( 'Mobile Menu', 'chcd-theme' ),
			'footer'   => __( 'Footer Menu', 'chcd-theme' ),
			'social'   => __( 'Social Menu', 'chcd-theme' ),
		];
	...
	public function widgets() {
		...
		register_sidebar(
			array_merge(
				$shared_args,
				[
					'name'        => __( 'Footer #1', 'chcd-theme' ),
					'id'          => 'sidebar-1',
					'description' => __( 'Widgets in this area will be displayed in the first column in the footer.', 'chcd-theme' ),
				]
			)
		);
		...
		register_sidebar(
			array_merge(
				$shared_args,
				[
					'name'        => __( 'Footer #2', 'chcd-theme' ),
					'id'          => 'sidebar-2',
					'description' => __( 'Widgets in this area will be displayed in the second column in the footer.', 'chcd-theme' ),
				]
			)
		);
	}
	...
	public function frontend_scripts() {
		...
		wp_enqueue_script( 'chcd-js', get_theme_file_uri( 'assets/js/index.min.js' ), [], $theme_version, false );
		wp_script_add_data( 'chcd-js', 'async', true );

		// Tabbed content script.
		wp_enqueue_script( 'chcd-tabs', get_theme_file_uri( 'assets/js/theme-tabs.min.js' ), [ 'jquery' ], $theme_version, true );
	}
	...
	public function frontend_styles() {
		 ...
		 */
		wp_enqueue_style( 'chcd-theme', get_theme_file_uri( 'assets/css/style.min.css' ), [], $theme_version, 'all' );

		// Right-to-left languages.
		if ( is_rtl() ) {
			wp_enqueue_style( 'chcd-rtl', get_theme_file_uri( 'assets/css/rtl.min.css' ), [ 'chcd-theme' ], $theme_version, 'all' );
		}

		wp_enqueue_style( 'chcd-style', get_stylesheet_uri(), [ 'chcd-theme' ], $theme_version, 'all' );
		wp_style_add_data( 'chcd-style', 'rtl', 'replace' );

		// Add print CSS.
		wp_enqueue_style( 'chcd-print-style', get_theme_file_uri( 'print.css' ), [], $theme_version, 'print' );
		 ...
		 */
		wp_enqueue_style( 'chcd-dark', get_theme_file_uri( 'assets/css/dark-mode.min.css' ), [ 'chcd-theme' ], $theme_version, '(prefers-color-scheme: dark)' );

		// Toolbar styles.
		wp_enqueue_style( 'chcd-toolbar', get_theme_file_uri( 'assets/css/toolbar.min.css' ), [ 'chcd-theme' ], $theme_version, 'all' );
	}
	...
	public function backend_styles() {
		...
		// Admin page styles.
		wp_enqueue_style( 'chcd-admin', get_theme_file_uri( 'assets/css/admin.css' ), [], $theme_version, 'all' );

		// WordPress 5.4 + styles.
		if ( version_compare( $wp_version, '5.4', '>=' ) ) {
			wp_enqueue_style( 'chcd-edit-page', get_theme_file_uri( 'assets/css/edit-page.css' ), [], $theme_version, 'all' );
		} else {
			wp_enqueue_style( 'chcd-edit-page', get_theme_file_uri( 'assets/css/edit-page-early.css' ), [], $theme_version, 'all' );
		}

		// Right-to-left languages.
		if ( is_rtl() ) {
			wp_enqueue_style( 'chcd-admin-rtl', get_theme_file_uri( 'assets/css/admin-rtl.css' ), [ 'chcd-admin' ], $theme_version, 'all' );
		}

		// Toolbar styles.
		wp_enqueue_style( 'chcd-toolbar', get_theme_file_uri( 'assets/css/toolbar.css' ), [ 'admin-bar' ], $theme_version, 'all' );
	}
	...
	public function editor_customizer_styles( $mce_init ) {
		...
		$styles = chcd_get_customizer_css( 'classic-editor' );
	...
	public function block_editor_styles() {
		...
		wp_enqueue_style( 'chcd-block-editor-styles', get_theme_file_uri( '/assets/css/editor-style-block.css' ), [], wp_get_theme()->get( 'Version' ), 'all' );
		wp_style_add_data( 'chcd-block-editor-styles', 'rtl', 'replace' );

		// Add inline style from the Customizer.
		wp_add_inline_style( 'chcd-block-editor-styles', chcd_get_customizer_css( 'block-editor' ) );

		// Add inline style for non-latin fonts.
		wp_add_inline_style( 'chcd-block-editor-styles', Non_Latin :: get_non_latin_css( 'block-editor' ) );

		// Enqueue the editor script.
		wp_enqueue_script( 'chcd-block-editor-script', get_theme_file_uri( '/assets/js/editor-script-block.js' ), [ 'wp-blocks', 'wp-dom' ], wp_get_theme()->get( 'Version' ), true );
	}
	...
	public function customize_scripts() {
		...
		wp_enqueue_script( 'chcd-customize', get_theme_file_uri( 'assets/js/customize.min.js' ), [], $theme_version, true );
	}
	...
	public function customize_styles() {
		...
		wp_enqueue_script( 'chcd-customize', get_theme_file_uri( '/assets/js/customize.js' ), [ 'jquery' ], $theme_version, false );

		// Add script for color calculations.
		wp_enqueue_script( 'chcd-color-calculations', get_theme_file_uri( '/assets/js/color-calculations.js' ), [ 'wp-color-picker' ], $theme_version, false );

		// Add script for controls.
		wp_enqueue_script( 'chcd-customize-controls', get_theme_file_uri( '/assets/js/customize-controls.js' ), [ 'chcd-color-calculations', 'customize-controls', 'underscore', 'jquery' ], $theme_version, false );
		wp_localize_script( 'chcd-customize-controls', 'twentyTwentyBgColors', $this->get_customizer_color_vars() );

		wp_enqueue_style( 'chcd-customize', get_theme_file_uri( 'assets/css/customize.min.css' ), [], $theme_version, 'all' );
	}
	...
	public function customize_preview_init() {
		...
		wp_enqueue_script( 'chcd-customize-preview', get_theme_file_uri( '/assets/js/customize-preview.js' ), [ 'customize-preview', 'customize-selective-refresh', 'jquery' ], $theme_version, true );
		wp_localize_script( 'chcd-customize-preview', 'twentyTwentyBgColors', $this->get_customizer_color_vars() );
		wp_localize_script( 'chcd-customize-preview', 'twentyTwentyPreviewEls', $this->get_elements_array() );

		wp_add_inline_script(
			'chcd-customize-preview',
			sprintf(
				'wp.customize.selectiveRefresh.partialConstructor[ %1$s ].prototype.attrs = %2$s;',
				wp_json_encode( 'cover_opacity' ),
				wp_json_encode( chcd_customize_opacity_range() )
			)
		);
	}
	...
	public function login_styles() {
		...
		wp_enqueue_style( 'chcd-login', get_theme_file_uri( 'assets/css/login.min.css' ), [], $theme_version, 'all' );
	}
	...
	public function get_elements_array() {
		...
		*/
		return apply_filters( 'chcd_get_elements_array', $elements );
	}
	...
	public function skip_link() {
		echo '<a class="skip-link screen-reader-text" href="#site-content">' . __( 'Skip to the content', 'chcd-theme' ) . '</a>';
	}
	...
	public function theme_page() {
		...
		$this->theme_page = add_theme_page(
			__( 'Wilbur Theme Information', 'chcd-theme' ),
			__( 'Wilbur Theme', 'chcd-theme' ),
			'manage_options',
			'chcd-theme',
