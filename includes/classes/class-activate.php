<?php
/**
 * Courtney theme activation class
 *
...
namespace Courtney_Theme\Classes;
...
class Activate {
	...
	public function theme_switch() {
		...
		$needs_update = version_compare( phpversion(), CHCD_PHP_VERSION, '<' );
	...
	?>
		<div id="chcd-theme-php-notice" class="notice notice-error is-dismissible">
			<?php echo sprintf(
				'<p>%1s %2s %3s %4s</p>',
				__( 'The Wilbur theme could not be activated because it requires PHP version', 'chcd-theme' ),
				CHCD_PHP_VERSION,
				__( 'or greater. Your system is running PHP version', 'chcd-theme' ),
				phpversion()
			); ?>
		</div>
	<?php
    ...
    private function redirect() {
		...
		if ( 'themes.php' == $pagenow && is_admin() && isset( $_GET['activated'] ) ) {
			...
			if ( current_user_can( 'customize' ) ) {
				wp_redirect( admin_url( 'themes.php' ) . '?page=chcd-theme' );
			}
		}
